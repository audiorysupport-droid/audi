...
// ============================================================
// ROYALTY BALANCE HELPERS - STAGE 3
// ============================================================

function generateRoyaltyBalanceId() {
  return `royalty_balance_${crypto.randomUUID()}`;
}


function getRoyaltyBalancePayoutStatus(
  unpaidAmount,
  minimumPayout
) {
...
async function getRoyaltyBalance(
  env,
  userId,
  recipientType,
  recipientId,
...
async function rebuildRoyaltyBalance(
  env,
  userId,
  recipientType,
  recipientId,
  ...
  const totals = ledgerResult || {};

  const totalEarned = Number(
    totals.total_earned || 0
  );
  ...
  const totalHeld = Number(
    totals.total_held || 0
  );

  const totalAdjustments = Number(
    totals.total_adjustments || 0
  );

  const totalUnpaid = Number(
    totals.total_unpaid || 0
  );

  const minimumPayout = 0;

  const payoutStatus =
    getRoyaltyBalancePayoutStatus(
      totalUnpaid,
      minimumPayout
    );
  ...
  if (existing) {
    await env.DB.prepare(`
      ...
      WHERE id = ?
        AND user_id = ?
    `).bind(
      totals.recipient_name || existing.recipient_name,
      totalEarned,
      totalPaid,
      totalHeld,
      totalAdjustments,
      ...
      minimumPayout,
      payoutStatus,
      totals.last_earned_at || null,
      totals.last_paid_at || null,
      existing.id,
      userId
    ).run();

    return await getRoyaltyBalance(
      env,
      userId,
      recipientType,
      recipientId,
  ...
  const balanceId =
    generateRoyaltyBalanceId();

  await env.DB.prepare(`
    INSERT INTO royalty_balances (
      id,
      user_id,
      recipient_type,
      recipient_id,
...
// ============================================================
// ROYALTY STATEMENT HELPERS
// ============================================================

function generateRoyaltyStatementId() {
  return `royalty_statement_${crypto.randomUUID()}`;
}

function generateRoyaltyStatementItemId() {
  return `royalty_statement_item_${crypto.randomUUID()}`;
}

function isValidStatementDate(value) {
  return /^\d{4}-\d{2}-\d{2}$/.test(value);
}
...
function getStatementEarningDate(row) {
  return (
    row.earning_date ||
    row.event_date ||
...
async function getRoyaltyStatement(
  env,
  userId,
  statementId
) {
  const result = await env.DB.prepare(`
    SELECT
      ...
      COUNT(rsi.id) AS transaction_count

    FROM royalty_statements rs

    LEFT JOIN royalty_statement_items rsi
      ON rsi.statement_id = rs.id
  ...
  return {
    ...
    total_adjustments:
      Number(
        Number(result.total_adjustments || 0)
          .toFixed(6)
      ),

    total_held:
      Number(
        Number(result.total_held || 0)
          .toFixed(6)
      ),

    total_unpaid:
      Number(
        Number(result.total_unpaid || 0)
          .toFixed(6)
      )
  };
}
...
async function getRoyaltyStatementItems(
  env,
  userId,
  statementId
) {
  const result = await env.DB.prepare(`
    ...
    FROM royalty_statement_items rsi

    LEFT JOIN releases r
      ON r.id = rsi.release_id

    LEFT JOIN tracks t
      ON t.id = rsi.track_id

    LEFT JOIN sales_events se
      ON se.id = rsi.sales_event_id

    LEFT JOIN royalty_calculations rc
      ON rc.id = rsi.calculation_id

    INNER JOIN royalty_statements rs
      ON rs.id = rsi.statement_id

    WHERE rsi.statement_id = ?
      AND rs.user_id = ?

    ORDER BY
      rsi.earning_date ASC,
      rsi.created_at ASC
  `)
    .bind(
      statementId,
  ...
  return (result.results || []).map(item => ({
    ...
    split_percentage:
      Number(
        Number(item.split_percentage || 0)
          .toFixed(6)
      ),

    royalty_base:
      Number(
        Number(item.royalty_base || 0)
          .toFixed(6)
      ),

    royalty_amount:
      Number(
        Number(item.royalty_amount || 0)
          .toFixed(6)
      )
  }));
}

const ROYALTY_PAYOUT_STATUSES = [
  "pending",
  "processing",
  "paid",
  "failed",
...
function generateRoyaltyPayoutId() {
  return `royalty_payout_${crypto.randomUUID()}`;
}


function generateRoyaltyPayoutItemId() {
  return `royalty_payout_item_${crypto.randomUUID()}`;
}


function isValidRoyaltyPayoutStatus(value) {
  return ROYALTY_PAYOUT_STATUSES.includes(value);
}


function isValidRoyaltyPayoutMethod(value) {
  return ROYALTY_PAYOUT_METHODS.includes(value);
}


async function getRoyaltyPayout(
  env,
  userId,
  payoutId
) {
  const result = await env.DB.prepare(`
    SELECT
      ...
      COUNT(rpi.id) AS transaction_count

    FROM royalty_payouts rp

    LEFT JOIN royalty_payout_items rpi
      ON rpi.payout_id = rp.id
...
async function getRoyaltyPayoutItems(
  env,
  userId,
  payoutId
) {
  const result = await env.DB.prepare(`
    ...
    FROM royalty_payout_items rpi

    INNER JOIN royalty_ledger rl
      ON rl.id = rpi.ledger_id

    LEFT JOIN releases r
      ON r.id = rl.release_id

    LEFT JOIN tracks t
      ON t.id = rl.track_id

    INNER JOIN royalty_payouts rp
      ON rp.id = rpi.payout_id
...
// ============================================================
// POST /v1/royalties/balances/rebuild
// REBUILD ROYALTY BALANCES FROM LEDGER
// ============================================================

if (
  request.method === "POST" &&
  url.pathname ===
    "/v1/royalties/balances/rebuild"
) {
  try {
    const auth =
      await requireRoyaltyAuth(
    ...
    const recipients =
      recipientsResult.results || [];

    const balances = [];

    for (const recipient of recipients) {
      const balance =
        await rebuildRoyaltyBalance(
          env,
          userId,
    ...
    return json({
      success: true,
      message:
        "Royalty balances rebuilt successfully",
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to rebuild royalty balances"
    }, 500);
  }
}

// ============================================================
// GET /v1/royalties/balances
// LIST ROYALTY BALANCES
// ============================================================

if (
  request.method === "GET" &&
  url.pathname ===
    "/v1/royalties/balances"
) {
  try {
    const auth =
      await requireRoyaltyAuth(
    ...
    const result =
      await env.DB.prepare(`
        ...
        WHERE ${conditions.join(" AND ")}
        ORDER BY
          rb.total_unpaid DESC,
          rb.recipient_name ASC
      `).bind(...params).all();
    ...
    for (const total of Object.values(
      ...
      total.total_earned =
        roundMoney(total.total_earned);

      total.total_paid =
        roundMoney(total.total_paid);

      total.total_held =
        roundMoney(total.total_held);
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to load royalty balances"
    }, 500);
  }
}

// ============================================================
// GET /v1/royalties/balances/:recipient_id
// GET INDIVIDUAL ROYALTY BALANCE
// ============================================================

const balanceRecipientMatch =
  url.pathname.match(
    /^\/v1\/royalties\/balances\/([^/]+)$/
  );
...
if (
  ...
  try {
    ...
    if (!result) {
      return json({
        success: false,
        error: "Royalty balance not found",
        recipient_id: recipientId,
        currency
      }, 404);
    }
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to load royalty balance"
    }, 500);
  }
}

// ============================================================
// GET /v1/royalties/balances/:recipient_id/transactions
// ROYALTY LEDGER TRANSACTIONS FOR RECIPIENT
// ============================================================

const balanceTransactionsMatch =
  url.pathname.match(
    /^\/v1\/royalties\/balances\/([^/]+)\/transactions$/
  );

if (
  request.method === "GET" &&
  balanceTransactionsMatch
) {
  try {
    const auth =
      await requireRoyaltyAuth(
    ...
    const transactions =
      (result.results || [])
        .map(transaction => ({
          ...
          split_percentage:
            Number(
              transaction.split_percentage || 0
            ),

          royalty_base:
            Number(
              Number(
                transaction.royalty_base || 0
              ).toFixed(6)
            ),
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to load royalty transactions"
    }, 500);
  }
}

// ============================================================
// POST /v1/royalties/statements/generate
// GENERATE ROYALTY STATEMENT
// ============================================================

if (
  request.method === "POST" &&
  url.pathname === "/v1/royalties/statements/generate"
) {
  try {
    ...
    if (!recipientType) {
      return json({
        success: false,
        error: "recipient_type is required"
      }, 400);
    }
    ...
    if (
      ...
      return json({
        success: false,
        error:
          "recipient_type must be artist or contributor"
      }, 400);
    }
    ...
    if (!isValidStatementDate(periodStart)) {
      return json({
        success: false,
        error:
    ...
    if (!isValidStatementDate(periodEnd)) {
      return json({
        success: false,
        error:
    ...
    if (!recipient) {
      return json({
        success: false,
        error: "Royalty recipient not found"
      }, 404);
    }
    ...
    if (existingStatement) {
      ...
      const transactions =
        await getRoyaltyStatementItems(
          env,
          userId,
          existingStatement.id
        );
      ...
      return json({
        ...
        duplicate: true,
        message:
          "Royalty statement already exists",
        statement,
        transactions
      });
    }
    ...
    const ledgerResult =
      await env.DB
        .prepare(`
          ...
          LEFT JOIN sales_events se
            ON se.id = rl.sales_event_id

          LEFT JOIN royalty_calculations rc
            ON rc.id = rl.calculation_id

          WHERE rl.user_id = ?
            AND rl.recipient_type = ?
            AND rl.recipient_id = ?
            AND rl.currency = ?
    ...
    //
    // IMPORTANT:
    // Keep 6 decimal places internally.
    // Do not round individual ledger amounts to cents.
    // --------------------------------------------------------
    ...
    for (const entry of ledgerEntries) {
      ...
      const amount =
        Number(entry.royalty_amount || 0);

      if (
        entry.entry_type === "royalty"
      ) {
        totalEarned += amount;
      }
    ...
    // Financial precision:
    // store/report six decimal places.
    totalEarned =
      Number(totalEarned.toFixed(6));

    totalPaid =
      Number(totalPaid.toFixed(6));

    totalHeld =
      Number(totalHeld.toFixed(6));

    totalAdjustments =
      Number(totalAdjustments.toFixed(6));

    totalUnpaid =
      Number(totalUnpaid.toFixed(6));
    ...
    // --------------------------------------------------------
    // Create statement transaction items
    // --------------------------------------------------------

    for (const entry of ledgerEntries) {

      const earningDate =
        getStatementEarningDate(
          entry
        );

      const itemId =
        generateRoyaltyStatementItemId();

      await env.DB
        .prepare(`
          INSERT INTO royalty_statement_items (
            id,
            statement_id,
            ledger_id,
            sales_event_id,
        ...
        .bind(
          ...
          statementId,
          entry.id,
          entry.sales_event_id || null,
          entry.calculation_id || null,
          entry.release_id || null,
          entry.track_id || null,
          ...
          Number(
            entry.split_percentage || 0
          ),
          Number(
            entry.royalty_base || 0
          ),
          Number(
            entry.royalty_amount || 0
          ),
    ...
    return json({
      success: true,
      message:
        "Royalty statement generated successfully",
      statement,
      transactions
    }, 201);
    ...
    console.error(
      "Generate royalty statement error:",
      error
    );
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to generate royalty statement"
    }, 500);
  }
}
...
if (
  request.method === "GET" &&
  url.pathname === "/v1/royalties/statements"
) {
  try {
    const auth = await requireRoyaltyAuth(request, env);
    const userId = auth.userId;
    ...
    if (
      recipientType &&
      !["artist", "contributor"].includes(recipientType)
    ) {
      return json({
        success: false,
    ...
    if (from && !isValidStatementDate(from)) {
      return json({
        success: false,
        error: "Invalid from date. Expected YYYY-MM-DD"
      }, 400);
    ...
    const normalizedCurrency =
      currency
        ? normalizeStatementCurrency(currency)
        : null;
    ...
    return json({
      ...
      pagination: {
        ...
        has_next:
          offset + statements.length < total,
    ...
    console.error(
      "GET /v1/royalties/statements error:",
      error
    );
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to fetch royalty statements"
    }, 500);
  }
}
...
if (
  request.method === "GET" &&
  /^\/v1\/royalties\/statements\/[^/]+$/.test(
    url.pathname
  )
) {
  try {
    const auth = await requireRoyaltyAuth(
      request,
      env
    );
    ...
    if (!statement) {
      return json({
        success: false,
        error: "Royalty statement not found"
      }, 404);
    }
    ...
    console.error(
      "GET /v1/royalties/statements/:statement_id error:",
      error
    );
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to fetch royalty statement"
    }, 500);
  }
}
...
if (
  request.method === "GET" &&
  /^\/v1\/royalties\/statements\/[^/]+\/transactions$/.test(
    url.pathname
  )
) {
  try {
    ...
    return json({
      ...
      summary: {
        ...
        total_royalty_base:
          Number(
            totalRoyaltyBase.toFixed(6)
          ),

        total_royalty_amount:
          Number(
            totalRoyaltyAmount.toFixed(6)
          )
      }
    });
    ...
    console.error(
      "GET /v1/royalties/statements/:statement_id/transactions error:",
      error
    );
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to fetch royalty statement transactions"
    }, 500);
  }
}
...
if (
  request.method === "GET" &&
  /^\/v1\/royalties\/payouts\/[^/]+\/transactions$/.test(
    url.pathname
  )
) {
  try {
    ...
    if (!payout) {
      return json({
        success: false,
        error:
          "Royalty payout not found"
      }, 404);
    }


    const transactions =
      await getRoyaltyPayoutItems(
        env,
        userId,
        payoutId
      );
    ...
    console.error(
      "Get royalty payout transactions error:",
      error
    );
...
if (
  request.method === "POST" &&
  url.pathname === "/v1/royalties/payouts"
) {
  try {
    const auth =
      await requireRoyaltyAuth(
    ...
    if (!ledgerIds.length) {
      return json({
        success: false,
        error:
          "ledger_ids must contain at least one royalty ledger ID"
      }, 400);
    }
    ...
    if (
      ...
      uniqueLedgerIds.length
    ) {
      return json({
        success: false,
        error:
          "One or more royalty ledger entries were not found"
      }, 404);
    }
    ...
    for (const ledger of ledgerEntries) {
      ...
      if (
        ledger.recipient_type !==
        recipientType
      ) {
        return json({
          success: false,
          error:
            "All ledger entries must belong to the requested recipient type",
          ledger_id:
            ledger.id
        }, 422);
      }
      ...
      if (
        ...
        return json({
          success: false,
          error:
            "All ledger entries must belong to the requested recipient",
          ledger_id:
            ledger.id
        }, 422);
      }
      ...
      if (
        ...
        return json({
          success: false,
          error:
            "All ledger entries must use the requested currency",
          ledger_id:
            ledger.id
        }, 422);
      }
      ...
      if (
        ...
        return json({
          success: false,
          error:
            "Only unpaid royalty ledger entries can be paid",
          ledger_id:
            ledger.id,
          current_status:
            ledger.status
      ...
      // ------------------------------------
      // Extra protection:
      // ledger cannot already belong
      // to another payout
      // ------------------------------------

      const existingPayoutItem =
        await env.DB.prepare(`
          SELECT
            rpi.id,
            rpi.payout_id,
      ...
      if (existingPayoutItem) {
        return json({
          success: false,
          error:
            "One or more ledger entries are already attached to a payout",
          ledger_id:
            ledger.id,
          payout_id:
            existingPayoutItem.payout_id,
          payout_status:
            existingPayoutItem.payout_status
        }, 409);
      }
    }
    ...
    const recipientName =
      ledgerEntries[0].recipient_name ||
      "Unknown recipient";
    ...
    for (const ledger of ledgerEntries) {
      ...
      const itemId =
        generateRoyaltyPayoutItemId();

      await env.DB.prepare(`
        INSERT INTO royalty_payout_items (
          id,
          payout_id,
          ledger_id,
          user_id,
    ...
    return json({
      ...
      message:
        "Royalty payout created successfully",
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to create royalty payout"
    }, 500);
  }
}
...
if (
  ...
  try {
    ...
    return json({
      ...
      error:
        error?.message ||
        "Failed to load royalty payouts"
    }, 500);
  }
}
...
if (
  request.method === "POST" &&
  /^\/v1\/royalties\/payouts\/[^/]+\/process$/.test(
    url.pathname
  )
) {
  try {
    ...
    if (
      ...
      return json({
        success: false,
        error:
          "Payout cannot be processed in its current status",
        status:
          payout.status
      }, 409);
    }
    ...
    if (!transactions.length) {
      return json({
        success: false,
        error:
          "Payout has no transactions"
      }, 422);
    }


    // ----------------------------------------
    // Verify every ledger entry
    // ----------------------------------------
    ...
    for (
      ...
      if (
        ...
        return json({
          success: false,
          error:
            "A payout transaction is no longer unpaid",
          ledger_id:
            ledger.id,
          current_status:
            ledger.status
    ...
    try {
      ...
      const externalReference =
        `sandbox_payout_${crypto.randomUUID()}`;
      ...
      return json({
        ...
        message:
          "Royalty payout processed successfully",
      ...
      console.error(
        "Royalty payout processing error:",
        processingError
      );
      ...
      return json({
        ...
        error:
          "Royalty payout processing failed",
        ...
        details:
          processingError?.message ||
          "Unknown payout processing error"
      }, 500);
    }
